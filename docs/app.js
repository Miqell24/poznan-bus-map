...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: GTFS ZTM Poznań' }));
  ...
  function applyFilters() {
    ...
    if (state.bus && !state.tram) {
      ...
      numC = ['all', modeC, selC];
      numField = state.tram && !state.bus ? tramOnlyNumbersN : numberFieldN;
    }
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · GTFS: ZTM Poznań';
      ...
      a.download = `poznan-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `poznan-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // NOT fitBounds(meta.bbox): the ZTM network reaches Murowana Goślina, Kórnik and
  // Stęszew, so fitting the data bbox would open on countryside with the city as a
  // blob. Open on the city — the whole network is one zoom out.
  map.jumpTo({ center: [16.925, 52.405], zoom: 11.5 });
}
