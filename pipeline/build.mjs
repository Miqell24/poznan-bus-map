...
// as in Poznań — both modes read ONE feed and are separated by route_type.
...
// m — longer jumps between shape points are GTFS data gaps. The ZTM Poznań feed
// samples shapes at ~42 m median (p90 164 m, holes up to 2.8 km), so the threshold
...
// routeTypes splits the single ZTM feed: 3 = bus (11 = trolleybus, none here),
// 0 = tram. Without it `--all` on the bus mode would swallow the tram lines too.
const MODES = [{
  mode: 'bus', label: 'buses', gtfsDir: 'data/gtfs', osmFile: 'data/osm/poznan.json',
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f', routeTypes: ['3', '11'],
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['160']),
}];
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
if (tramLines.length) MODES.push({
  mode: 'tram', label: 'trams', gtfsDir: 'data/gtfs', osmFile: 'data/osm/poznan-tram.json',
...
async function processMode(cfg) {
  ...
  // direction_id alone can lie (the Athens/OSY defect): 4 lines here (125,
  // 151, 580, 822) carry one constant value on ALL their trips while the
  ...
  // and only the busier one gets drawn. Pre-scan direction diversity per
  // line; where direction_id cannot tell directions apart, the headsign is
  // the key.
...
await (await import('./night.mjs')).nightPass(outDir, /^2\d\d$/, { sort: false });
